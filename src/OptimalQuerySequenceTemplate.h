...
template <class... Args> // Should contain BlockQuery structures only
...
public:
    template <class Assigner_t>
    static void assign(Assigner_t& assigner) {
        assigner.addQueryGlobal(BlockQuery_t::blockA, BlockQuery_t::blockB, OffsetA, OffsetB);
    }
};
...
private:
    ...
    typedef assign_global_exec<BlockQuery_t, offsetA+offsetB==k?BlockQuery_t::blockA==BlockQuery_t::blockB-1?0:offsetA:offsetA,
    offsetA+offsetB==k?0:offsetB> Assignment;
...
//Main Loop
template<class BlockMeta_t, class QuerySequence_t, unsigned int current, unsigned int last, unsigned int k>
struct assign_global_loop {
private:
    typedef assign_global<BlockMeta_t,typename vlist::first<QuerySequence_t>::type, k> Head;
    typedef assign_global_loop<BlockMeta_t,typename vlist::remove_first<QuerySequence_t>::type, current+1, last, k> Tail;

public:
    template <class Assigner_t>
    static void assign(Assigner_t& assigner) {
        Head::assign(assigner);
        Tail::assign(assigner);
    }
};

//End Loop
template<class BlockMeta_t, class QuerySequence_t, unsigned int last, unsigned int k>
struct assign_global_loop<BlockMeta_t, QuerySequence_t, last, last, k>{
private:
public:
    template <class Assigner_t>
    static void assign(Assigner_t&) {}
...
private:
    typedef assign_global_loop<BlockMeta_t, QuerySequence_t, 0u, size, k> Loop;

public:
    template <class Assigner_t>
    static void assign(Assigner_t& assigner) {
        Loop::assign(assigner);
    }

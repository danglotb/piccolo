...
QueryBuilder::QueryBuilder(BlockId blockA, BlockId blockB, unsigned int offset) : /*BlockA(blockA), BlockB(blockB),*/
    ...
    BlockMaskA(BLOCK_MASK_AT(blockA)), BlockMaskB(BLOCK_MASK_AT(blockB)), m_queryMeta(offset - SizeA), E(0) {
	m_lastQuery.setBlockIds(blockA, blockB);
}

QueryBuilder::QueryBuilder(BlockId blockA, BlockId blockB, unsigned int offset, int e) :
    SizeA(BLOCK_SIZE_AT(blockA)), OffsetB(offset), OffsetEndB(OffsetB + BLOCK_SIZE_AT(blockB)),
    BlockMaskA(BLOCK_MASK_AT(blockA)), BlockMaskB(BLOCK_MASK_AT(blockB)), m_queryMeta(offset - SizeA), E(e) {
    m_lastQuery.setBlockIds(blockA, blockB);
    std::cout << "build with e=" << e << std::endl;
}

...
bool parseParameters(int argc, char const* argv[], Parameters& parameters) {
    ...
    std::array<char const*, 1> intOptions({"-e"});
    std::array<unsigned int *, 1> intOptionsOutput({&parameters.erreur});
    int currentOption = -1;
    bool optInt = false;
    for (int it = 1; it < argc; ++it) {
        bool optionFound = false;
        for (int i = 0; i< (int)intOptions.size();++i){
            if (std::strcmp(argv[it], intOptions[i]) == 0) {
                if (currentOption != -1)
                    return false;
                currentOption = i;
                optionFound = true;
        ...
        if (optionFound)
            continue;
        if (currentOption != -1 && optInt) {
            optInt = false;
            *intOptionsOutput[currentOption] = std::stoi(argv[it]);
            currentOption = -1;
            continue;
        }
        else {
...
void run(RnaDataBase const& sequences, RnaIndex const& index, std::ostream& out, Parameters const& parameters) {
    ...
    RnaMatch matcher(index, parameters.erreur);
...
int main(int argc, char const* argv[]) {
    ...
    if (!parseParameters(argc, argv, parameters)) {
        ...
        std::cout << "\t-t2f : Will convert a tabular file into fasta, need to specify the input with -i <in_path> and the output with -o <out_path>" << std::endl;
        return EXIT_SUCCESS;
    }
